...
public class GameEngine implements Runnable, CollisionSystem.CollisionCallback {
    ...
    private void updatePlaying() {
        ...
        if (!dave.isDead()) {
            ...
            boolean jumpPressed = input.isKeyPressed(KeyEvent.VK_UP) || input.isKeyPressed(KeyEvent.VK_W) || input.isKeyPressed(KeyEvent.VK_SPACE);
            boolean shootPressed = input.isKeyPressed(KeyEvent.VK_CONTROL) || input.isKeyPressed(KeyEvent.VK_F) || input.isKeyPressed(KeyEvent.VK_SPACE);
            boolean jetpackPressed = input.isKeyPressed(KeyEvent.VK_ALT) || input.isKeyPressed(KeyEvent.VK_J);
            ...
            if (dave.isJetpackActive()) {
                ...
                // Jump initiation (only allowed if grounded and fresh keypress)
                if (jumpPressed && dave.isGrounded()) {
                    if (dave.jump()) {
    ...
    private void updateLevelComplete() {
        levelCompleteTimer++;
        if (levelCompleteTimer >= 90 || input.isKeyPressed(KeyEvent.VK_ENTER) || input.isKeyPressed(KeyEvent.VK_SPACE)) {
    ...
    private void updateVictory() {
        if (input.isKeyPressed(KeyEvent.VK_ENTER) || input.isKeyPressed(KeyEvent.VK_R) || input.isKeyPressed(KeyEvent.VK_SPACE)) {
            stateMachine.transitionTo(GameState.MAIN_MENU);
        }
    }
    ...
    private void renderWorld(Graphics2D g) {
        ...
        int maxTileX = Math.min(currentLevel.getWidthInTiles() - 1, (int) ((camX + 320) / 16f) + 1);
        ...
        for (int ty = 0; ty < currentLevel.getHeightInTiles(); ty++) {
            for (int tx = minTileX; tx <= maxTileX; tx++) {
                ...
                if (type != TileType.EMPTY) {
                    ...
                    if (sprite != null) {
                        g.drawImage(sprite, (int) (tx * 16 - camX), ty * 16, null);
                    }
                }
            }
        }
        ...
        for (Collectible c : currentLevel.getCollectibles()) {
            if (!c.isCollected()) {
                ...
                if (itemImg != null) {
                    g.drawImage(itemImg, (int) (c.getX() - camX), (int) c.getY(), null);
                }
            }
        }
        ...
        for (Enemy enemy : currentLevel.getEnemies()) {
            if (enemy.isAlive() || enemy.isExploding()) {
                ...
                if (enemyImg != null) {
                    g.drawImage(enemyImg, (int) (enemy.getX() - camX), (int) enemy.getY(), null);
                }
            }
        }
        ...
        for (Projectile p : currentLevel.getProjectiles()) {
            if (p.isActive()) {
                BufferedImage pImg = p.isFromPlayer() ? sprites.getBulletSprite() : sprites.getEnemyShotSprite();
                if (pImg != null) {
                    g.drawImage(pImg, (int) (p.getX() - camX), (int) p.getY(), null);
                }
            }
        }
        ...
        if (daveImg != null) {
            g.drawImage(daveImg, (int) (dave.getX() - camX), (int) dave.getY(), null);
        }
    }
