...
public interface CollisionCallback {
    ...
    default void onScreenShake(int frames, float intensity) {}
}
    ...
    if (nextY > level.getPixelHeight() + 10f) {
        if (dave.die()) {
            if (callback != null) {
                ...
                callback.onScreenShake(12, 5.0f);
            }
        }
        return;
    }
...
public void checkEnemies(Dave dave, Level level) {
    ...
    for (Enemy enemy : level.getEnemies()) {
        if (enemy.isAlive() && playerBounds.intersects(enemy.getBounds())) {
            // FIX: Only destroy Dave, let enemy survive (classic behavior)
            // Uncomment next line if you want mutual destruction:
            // enemy.destroy();
            ...
            if (dave.die()) {
                if (callback != null) {
                    callback.onPlayerDied();
                    callback.onSoundTriggered(SoundEffect.EXPLODE);
                    callback.onScreenShake(10, 4.0f);
                }
            }
            return;
        }
...
FIX: Prevents tunneling through thin walls by checking intermediate positions.
*/
public void updateProjectiles(Dave dave, Level level) {
    ...
    for (int i = projectiles.size() - 1; i >= 0; i--) {
        ...
        if (p.getX() < 0 || p.getX() > level.getPixelWidth() || 
            p.getY() < 0 || p.getY() > level.getPixelHeight()) {
            p.deactivate();
            continue;
        }
        
        // FIX: Check collision against solid tiles with tunneling prevention
        boolean hitWall = false;
        float vx = p.isFromPlayer() ? 4.5f : -4.5f; // Approximate velocity magnitude
        float stepX = p.getVx() > 0 ? 1f : -1f;
        float checkX = p.getX() - p.getVx(); // Start from previous X position
        
        // Step through the path to prevent tunneling
        int steps = (int) Math.abs(p.getVx()) + 1;
        for (int step = 0; step < steps; step++) {
            int tx = (int) (checkX / 16f);
